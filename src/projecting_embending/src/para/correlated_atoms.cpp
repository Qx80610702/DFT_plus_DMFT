...
{ 
  int atoms_info::equ_atom(int iatom)
  {
    ...
    {
      std::cout << "Array equivalent_atom is out of range" << std::endl;
  ...
  int atoms_info::L(const int iatom)
  {
    if(iatom<0 || iatom>=this->n_DMFT_atoms)
    {
      std::cout << "Array angular_momment is out of range" << std::endl;
      std::exit(EXIT_FAILURE);
    }
  ...
  int atoms_info::iorb2ibasis(const int iorb)
  {
    ...
    {
      std::cout << "Array iorb_ibasis is out of range" <<std::endl;
      std::exit(EXIT_FAILURE);
    }
  ...
  double atoms_info::Uval(const int iatom)
  {
    if(iatom<0 || iatom>=this->n_DMFT_atoms)
    {
      std::cout << "Array Hubbard_U is out of range" <<std::endl;
      std::exit(EXIT_FAILURE);
    }
  ...
  double atoms_info::Jval(const int iatom)
  {
    if(iatom<0 || iatom>=this->n_DMFT_atoms)
    {
      std::cout << "Array Hund_J is out of range" <<std::endl;
      std::exit(EXIT_FAILURE);
    }
  ...
  int atoms_info::ineq_iatom(const int ineq)
  {
    ...
    {
      std::cout << "Array ineq_atom_to_iatom is out of range" <<std::endl;
      std::exit(EXIT_FAILURE);
    }
  ...
  int atoms_info::magnetic(const int iatom)
  {
    if(iatom<0 || iatom>=this->n_DMFT_atoms)
    {
      std::cout << "Array mag_moment is out of range" <<std::endl;
      std::exit(EXIT_FAILURE);
    }
  ...
  bool atoms_info::read_correlated_atoms_info()
  {
    ...
    while(ifs.good())
    {
      ...
      this->Hubbard_U[iatom] /= Hartree_to_eV;   //eV to Hartree
      this->Hund_J[iatom] /= Hartree_to_eV;      //eV to Hartree
      ...
      {
        std::cout << "Unsupported local symmetry!!!" << std::endl;
        std::exit(EXIT_FAILURE); 
      }
  ...
  bool atoms_info::read_local_symmetry(DMFT::input_info& in)
  {
    ...
    try {
      ...
      if(mpi_rank()==0) std::cout << "Warning: local_symmetry is not given and set default value 1" << std::endl;
      this->local_symmetry = 1;
    }
  ...
  void atoms_info::out()
  {
    ...
    for(int iatom=0; iatom<this->n_DMFT_atoms; iatom++)
    {
      ...
      ofs << std::setw(6) << std::fixed << std::setprecision(2) << this->Hubbard_U[iatom]*Hartree_to_eV
      << std::setw(6) << std::fixed << std::setprecision(2) << this->Hund_J[iatom]*Hartree_to_eV
