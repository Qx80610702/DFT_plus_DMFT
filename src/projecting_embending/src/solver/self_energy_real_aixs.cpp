...
namespace DMFT
{
        ...
        DFT_output::atoms_info& atom )
  {
    ...
    for(int ineq=0; ineq<ineq_num; ineq++)
    {
      ...
      while(ifs.good())
      {
        ...
        this->real_frequency[count] = omega/Hartree_to_eV;
      ...
      {
        std::cout << "The number of frequency points of Sigma_omega.dat is not equal to nomega" << std::endl;
        std::exit(EXIT_FAILURE);
      }
    }
  ...
  void self_energy_real_aixs::read_maxent_parms()
  {
    ...
    if (!ifs)  
	  {
	  	std::cout << "Error: fail to oepn maxent_params.dat!!!" << std::endl;
      std::exit(EXIT_FAILURE);
    }
    ifs.seekg(0);   //set the position at the beginning of the file
