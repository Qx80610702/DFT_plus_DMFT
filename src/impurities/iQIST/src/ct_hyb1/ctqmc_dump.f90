...
! close data file
     ...
     open(mytmp, file='Gt.dat', form='formatted', status='unknown')
     do j=1,ntime
       write(mytmp,'(f15.9\)') tmesh(j)
       do i=1,norbs
           write(mytmp,'(f15.9\)') gtau(j,i,i)
       enddo ! over j={1,ntime} loop
       write(mytmp, *) 
     enddo ! over i={1,norbs} loop
     close(mytmp)
...
! close data file
     close(mytmp)
      ...
      open(mytmp, file='H0t.dat', form='formatted', status='unknown')
      do j=1,ntime
        write(mytmp,'(f15.9\)') tmesh(j)
        do i=1,norbs
            write(mytmp,'(f15.9\)') htau(j,i,i)
        enddo 
        write(mytmp,*)
      enddo ! over i={1,norbs} loop
      close(mytmp)
...
! close data file
    ...
    open(mytmp, file='Gw.dat', form='formatted', status='unknown')
    do j=1,mfreq
      write(mytmp,'(f22.15\)') rmesh(j)
      do i=1,norbs
          write(mytmp,'(2f22.15\)') grnf(j,i,i)
      enddo ! over j={1,mfreq} loop
      write(mytmp,*)
    enddo ! over i={1,norbs} loop
    close(mytmp)
...
! close data file
    ...
    open(mytmp, file='H0w.dat', form='formatted', status='unknown')
    do j=1,mfreq
      write(mytmp,'(f17.9\)') rmesh(j)
      do i=1,norbs
          write(mytmp,'(2f17.9\)') hybf(j,i,i)
      enddo ! over j={1,mfreq} loop
      write(mytmp,*)
    enddo ! over i={1,norbs} loop
    close(mytmp)
...
! close data file
    ...
    open(mytmp, file='Sigma.dat', form='formatted', status='unknown')
    do j=1,mfreq
      write(mytmp,'(f22.15\)') rmesh(j)
      do i=1,norbs
          write(mytmp,'(2f22.15\)') sig2(j,i,i)
      enddo ! over j={1,mfreq} loop
      write(mytmp,*)
    enddo ! over i={1,norbs} loop
    close(mytmp)
