program maxentropy
  ...
  open(unit=1, file='maxent_input.dat', status='unknown')
  read(1, *) Nt
  read(1, *) idg
  read(1, *) deltag
  read(1, *) beta
  ...
  OPEN(UNIT=10,FILE='Gtau.dat',STATUS='old')
  DO it=1,nt
     READ(10,*) t(it),axt(it)
     if(idg.eq.0) then
  ...
  CALL main_maxent(a,w,axt,sxt,t,iflat,steps,beta,alpha0,nr,1.d0,nt,nw,dw)
...
!c------------------------------------------------------------------------------
SUBROUTINE main_maxent(a,w,axt,sxt,t,iflat,steps,beta,alpha0,nr,n0,nt,nw,dw)
  IMPLICIT NONE
  REAL*8, intent(inout):: a(-nw:nw), w(-nw:nw)
  REAL*8, intent(in)   :: axt(nt), sxt(nt), t(nt)
  INTEGER, intent(in)  :: iflat, steps, nr, nt, nw
  REAL*8, intent(in)   :: alpha0, n0, beta, dw
  ! locals
  REAL*8  :: ker(-nw:nw,nt)
...
SUBROUTINE MAXENT(a,rfac,alpha,temp,ker,sxt,axt,mm,f0,nt,nw,steps)
  ...
  REAL*8 :: aa1,aa2,de,p,mm1,mm2,arat,rand
...
!c------------------------------------------------------------------------------
SUBROUTINE SETMODEL(iflat,mm,nw,Dw)
  IMPLICIT NONE
  REAL*8,  intent(out):: mm(-nw:nw)
  INTEGER, intent(in) :: nw, iflat
  REAL*8,  intent(in) :: Dw
  ...
  select case (iflat) 
  case (0)
     print*, 'taking flat model'
     do i = -nw, nw
        mm(i) = 1
     end do
  case (1)
     print*, 'taking Gaussian model'
     do i = -nw, nw
        omega = Dw*i
        mm(i) = exp(-omega**2/dble(nw*Dw))
     end do
  case default
...
END SUBROUTINE INITKER
  ...
  subroutine RanGen_Ini
    ...
    ! Purpose
    ! =======
    !   Initialize the Random Number Generator. Here there are two Gernerators
    !   for choosing.
